...
$("document").ready(function() {
  ...
  $(".slider").each(function() {
    ...
    if (length > 0) {
      ...
      // ARROWS
      var $arrowLeft = $(
          '<div class="slider-arrow arrow-left"><i class="fa fa-chevron-left"></i></div>'
        ).appendTo($slider),
        $arrowRight = $(
          '<div class="slider-arrow arrow-right"><i class="fa fa-chevron-right"></i></div>'
        ).appendTo($slider);

      // DOTS
      var $sDots = $('<div class="slider-dots"/>').appendTo($slider);
      $frames.each(function(i) {
        var $dot = $('<div class="dot" data-num="' + i + '"/>').appendTo(
          $sDots
        );
      });
  ...
  // Scroll transform
  var ST = ScrollTransform('body');

  
  $(".scrollTransform").on('click',function(){
    ST.down();
  });
  //ST.force();
});